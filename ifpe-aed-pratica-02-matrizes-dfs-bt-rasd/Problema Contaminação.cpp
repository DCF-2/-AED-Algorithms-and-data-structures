...
bool checa_chuva(const vector<vector<char>> &C, int i, int j) {
    return (i > 0 && C[i - 1][j] == 'T') ||                                                      // Se há chuva diretamente acima
    (j > 0 && i < C.size() - 1 && C[i][j - 1] == 'T' && C[i + 1][j - 1] == 'X') ||              // Esquerda segura para fluxo de água
    (j < C[i].size() - 1 && i < C.size() - 1 && C[i][j + 1] == 'T' && C[i + 1][j + 1] == 'X'); // Direita segura para fluxo de água
}
    ...
    do {
        ...
        for (int i = 0; i < C.size(); i++) {         // Percorre todas as linhas
            for (int j = 0; j < C[i].size(); j++) { // Percorre todas as colunas
                if (C[i][j] != 'A') continue;      // Apenas células vazias podem ser preenchidas
...
bool valida(const vector<vector<char>> &C, int i, int j) {
    return (i >= 0 && j >= 0 && i < C.size() && j < C[i].size() && C[i][j] == 'A');
}
...
void chuva(vector<vector<char>> &C, int i, int j) {
    if (C[i][j] == 'T' || C[i][j] == 'A' ) {               
        ...
        C[i][j] = 'T';                                       // Marca a célula como molhada
        ...
        if (valida(C, i - 1, j)) chuva(C, i - 1, j); // Espalha para Cima
    }
}
...
void solucao_pilha(vector<vector<char>> &C, int x, int y) {
    ...
    while (!pilha.empty()) {                                // Enquanto houver células a processar
        ...
        if (C[i][j] == 'T'|| C[i][j] == 'A')  {               // Se já tem chuva ou pode ser preenchido
            C[i][j] = 'T';                                          // Marca a célula como molhada
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cin >> C[i][j];
            if (C[i][j] == 'T') {                           // Guarda a posição inicial da chuva
                x = i;
                y = j;
            }
        }
