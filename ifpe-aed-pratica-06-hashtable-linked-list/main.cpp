...
long hash_function<CPF>(CPF cpf) {
    string string_cpf = cpf.valor;
    long hash = 0;

    for(int i = 0; i < string_cpf.length(); i++){
        // CORREÇÃO: Usar isdigit() para verificar se o caractere é um dígito.
        if(isdigit(string_cpf[i])){
             hash = (hash * 31 ) + string_cpf[i];
        }
    }
        if (hash < 0){
        hash = -hash;
